...
class KMeansPlusPlus:
    ...
    def initialize_centroids(self, X, sample_weight=None):
        """
        ...
        for i in range(1, self.n_clusters):
            # Compute distances from the latest centroid to all points
            dist = torch.sum((X - centroids[i-1])**2, dim=1)
    ...
    def fit(self, X, sample_weight=None):
        ...
        self.initialize_centroids(X, sample_weight)
        ...
        for iteration in range(self.max_iter):
            ...
            self.loss = ((X - self.centroids[labels]) ** 2).sum(dim=-1).sqrt().sum().item()
            if self.logging:
                print(f"LOGGING: CLUSTERING LOSS {self.loss}")
            ...
            if centroid_shift < self.tol:
                if self.logging:
                    print(f"LOGGING: Converged at iteration {iteration}")
                break
        else:
